 ...
 * Time: 24:00
 */
class ReportOrganizerSurvey extends ControllerMember {

    public $data_activity, $data_parent_subject_assess;

    public function __construct() {
        $this->modelData = new TransactionActivity();
        $this->setTitle(lang('member.survey_of_activities'));
        $this->setBreadCrumb(array(lang('member.survey_of_activities') => ""));
        $this->search_filter = array(
            "name" => lang('transaction.type')
        );
        $this->orderBy = $this->modelData->getId() . " DESC";
        $this->indexUrl = IURLMemberConstant::REPORT_SURVEY_ORGANIZER_URL;
        $this->viewPath = IViewMemberConstant::REPORT_SURVEY_ORGANIZER_VIEW_INDEX;
        $this->setAutoCrud();
        parent::__construct();
    }

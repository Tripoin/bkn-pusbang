...
function FunctionLanguageName($value) {
    ...
    if (isset($_SESSION[LANGUAGE_SESSION])) {
        ...
        $db->connect();
        $db->select(
                $function->getEntity(), $function->getName(), array(), $function->getId() . EQUAL . "'" . $value[$function->getId()] . "'"
                . " AND " . $function->getLanguage() . EQUAL . "'" . $_SESSION[LANGUAGE_SESSION] . "'"
        );
...
function imageManager($title, $url, $link, $type) {
    ...
    if ($type == 1) {
        ...
        $txt = '<div class="col-sm-2 col-md-2">
                    ...
                    id="folder" action="' . $link . '" ondblclick="getFolder(this,event)" style="text-align: center;">
        ...
        $txt = '<div class="col-sm-2 col-md-2">
                    <a href="javascript:;"  ondblclick="viewPicture(\'' . $url . '\')" onclick="checkFile(this)" value="' . $title . '" value-type="0" name="fileimage[]" class="thumbnail" id="file" style="text-align: center;">

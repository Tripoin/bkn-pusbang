<form id="form-user" action="<?= URL('/page/member/user-profile/save') ?>" method="POST" class="form" onsubmit="return false;">
    <ul class="nav nav-tabs">
        ...
        <li role="presentation"><a href="#contactTab" data-toggle="tab"><?= lang('general.your_company'); ?></a></li>
    </ul>
    ...
    <div class="tab-content">
        <div class="tab-pane active" id="accountTab">
            <fieldset id="account">
                ...
                <?= Form()->id('province')->required(FALSE)->value($province)->data($data_province)->title(lang('general.province'))->placeholder(lang('general.province') . ' ....')->combobox(); ?>
                <?= Form()->id('city')->required(FALSE)->value($city)->data($data_city)->title(lang('general.city'))->placeholder(lang('general.city') . ' ....')->combobox(); ?>
                <?= Form()->id('district')->required(FALSE)->value($district)->data($data_district)->title(lang('general.district'))->placeholder(lang('general.district') . ' ....')->combobox(); ?>

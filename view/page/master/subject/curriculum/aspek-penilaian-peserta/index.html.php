...
$db->select($linkCurriculumAssess->getEntity(), ''
        . $masterCategoryAssess->getEntity() . DOT . $masterCategoryAssess->getId() . ' as id,'
        . $masterCategoryAssess->getEntity() . DOT . $masterCategoryAssess->getName() . ' as label', array($masterCategoryAssess->getEntity()), $linkCurriculumAssess->getEntity() . DOT . $linkCurriculumAssess->getCategoryAssessId() . EQUAL . $masterCategoryAssess->getEntity() . DOT . $masterCategoryAssess->getId()
        . " AND " . $linkCurriculumAssess->getEntity() . DOT . $linkCurriculumAssess->getCurriculumId() . equalToIgnoreCase($curriculumId)
);
...
$data_category_asses = getLov($masterCategoryAssess, $masterCategoryAssess->getParentId() . equalToIgnoreCase(3)
        .$str_query_cai);
...
<?= Form()->formFooter(null, null, "postFormAjaxPost('" . URL(getAdminTheme() . $this->indexUrl . '/curriculum/' . $subjectId . '/aspek-penilaian-peserta/save') . "')"); ?>
<script>
    $(function () {
...
//        postFormAjaxPost()
//        postAjaxEdit();
        $('.portlet-title > div > span').html('Aspek Penilaian Peserta Materi : <span style="color:black"><?= $_POST['subject_name']; ?></span>');
        initDetails();
    });

<?php

/* 
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

define('LANGUAGE_SESSION', 'language');

define('SESSION_USERNAME', 'username');
define('SESSION_FULLNAME', 'fullname');
define('SESSION_GROUP', 'group');
define('SESSION_EMAIL', 'email');
define('SESSION_TOKEN', 'token');
define('SESSION_LOCK_SCREEN', 'LOCK_SCREEN');

define('SESSION_ADMIN_USERNAME', 'username');
define('SESSION_ADMIN_FULLNAME', 'fullname');
define('SESSION_ADMIN_PASSWORD', 'admin_password');
define('SESSION_ADMIN_GROUP', 'group');
define('SESSION_ADMIN_EMAIL', 'email');
define('SESSION_ADMIN_TOKEN', 'token');
define('SESSION_ADMIN_LOCK_SCREEN', 'LOCK_SCREEN');
define('SESSION_ADMIN_AUTHORIZATION', 'TRIPOIN_CRYPT');

define('SESSION_ADMIN_AUTO_DATA', 'AUTO_DATA');

define('SESSION_CAPTCHA', 'captcha');